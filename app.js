...
// Load unique user ID from localStorage if available
const storedUserId = localStorage.getItem(CONFIG.STORAGE_KEYS.USER_ID);
if (storedUserId) {
    window.uniqueUserId = storedUserId;
    console.log('✓ Loaded unique user ID:', storedUserId);
} else {
    window.uniqueUserId = 'user_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
    localStorage.setItem(CONFIG.STORAGE_KEYS.USER_ID, window.uniqueUserId);
    console.log('✓ Generated new unique user ID:', window.uniqueUserId);
}

console.log('📱 Melo v1.0 - Loading...');
...
// Use MOOD_SCORE_MAP from CONFIG
const { MOOD_SCORE_MAP } = CONFIG;
...
function setupNavScroll() {
    ...
    window.addEventListener('scroll', () => {
        ...
        // Standard Auto-hide logic for Nav and Header
...
function changeView(viewName, saveToHistory = true) {
    ...
    // Cleanup any running processes from the current view
    cleanupCurrentView();
    ...
    if (nav) {
        if (viewName === 'login') {
            ...
            const isLoggedIn = localStorage.getItem(CONFIG.STORAGE_KEYS.LOGGED_IN) === 'true';
            if (isLoggedIn) {
                nav.classList.remove('nav-hidden');
            }
        }
...
function cleanupCurrentView() {
    ...
    if (typeof breathingExerciseInterval !== 'undefined' && breathingExerciseInterval) {
        clearInterval(breathingExerciseInterval);
        breathingExerciseInterval = null;
    }
    
    // Stop voice recognition/recording if running
    if (window.VoiceInputService && window.VoiceInputService.isListening) {
        window.VoiceInputService.stop();
    }
    if (window.VoiceRecorderService && window.VoiceRecorderService.isRecording) {
        window.VoiceRecorderService.stopRecording();
    }
    
    // Clear any pending timeouts that might disrupt UI
    isTyping = false;
}
...
function setupFirestoreListeners() {
    // Initialize Vent Room Socket
    if (typeof VentSocket !== 'undefined') {
        VentSocket.init((messages, isInit) => {
            if (isInit) {
                ventMessages = messages.map(m => ({
                    userId: m.userId,
                    text: m.text,
                ...
                if (ventMessages.length > 50) ventMessages.shift();
            }
            ...
            if (currentView === 'vent') {
                const countBadge = document.getElementById('vent-member-count');
                if (countBadge) countBadge.textContent = `${memberCount}/10 online`;
            }
        });
    }
    ...
    onSnapshot(auriQ, (snapshot) => {
        ...
        if (newHistory.length === 0) {
            newHistory.push({ ...WELCOME_MESSAGE });
        }
...
async function saveMessage(sender, text) {
    if (!window.firebaseImports || !window.db || !window.userId) {
        // Direct save to memory if Firebase not available
    ...
    try {
        ...
        // Snapshot listener in setupFirestoreListeners will update auriHistory and UI
    } catch (error) {
...
async function saveVentMessage(text) {
    if (typeof VentSocket !== 'undefined' && VentSocket.connected) {
        const success = VentSocket.sendMessage(text);
        if (success) {
    ...
    if (!window.firebaseImports || !window.db || !window.userId) {
        ...
        sessionStorage.setItem(CONFIG.STORAGE_KEYS.VENT_MESSAGES, JSON.stringify(ventMessages));
        showNotification('Vent saved locally!', 'info');
        if (currentView === 'vent') renderVentView();
        return;
    }
...
async function saveProfileData(data) {
    ...
    if (!window.db || !window.userId) {
        ...
        sessionStorage.setItem(CONFIG.STORAGE_KEYS.USER_PROFILE, JSON.stringify(userProfile));
...
async function generateAuriResponse(prompt) {
    ...
    // Therapy-Related & About Melo Responses
    ...
    if (lowerPrompt.includes('who built melo') || lowerPrompt.includes('who made melo') || lowerPrompt.includes('who created melo')) {
        await saveMessage('Auri', "Melo was built with love by a dedicated team: **Rucha Rathod**, **Pranjal Dabholkar**, and **Adien Pearce**. They are passionate about making mental wellness accessible to everyone! ❤️");
        return;
    }

    if (lowerPrompt.includes('what is melo') || lowerPrompt.includes('about melo')) {
        await saveMessage('Auri', "Melo is an AI-powered mental wellness platform designed specifically for Gen Z. We provide tools for journaling, toolkit exercises, and anonymous venting to help manage anxiety, depression, and social isolation. 🧠✨");
        return;
    }

    if (lowerPrompt.includes('anxious') || lowerPrompt.includes('anxiety')) {
        await saveMessage('Auri', "I'm sorry you're feeling anxious. Take a deep breath with me... In for 4, hold for 4, out for 4. 🧘‍♀️ You might also find our 'Breathing' exercise in the Toolkit helpful. Would you like to try it?");
        return;
    }

    if (lowerPrompt.includes('sad') || lowerPrompt.includes('depressed') || lowerPrompt.includes('lonely')) {
        await saveMessage('Auri', "I hear you, and it's okay to feel this way. You're not alone. I'm here to listen. Would you like to write about what's on your mind in your Journal, or maybe try a Grounding exercise?");
        return;
    }

    if (lowerPrompt.includes('happy') || lowerPrompt.includes('glad') || lowerPrompt.includes('good')) {
        await saveMessage('Auri', "That's wonderful to hear! I'm so glad you're having a good day. 🌟 Would you like to journal about what's making you feel this way so you can look back on it later?");
        return;
    }

    if (lowerPrompt.includes('thank') || lowerPrompt.includes('thanks')) {
        await saveMessage('Auri', "You're very welcome! I'm always here for you. Remember, you're doing great. 🌟");
        return;
    }
    ...
    // Kaggle-sourced Therapy FAQ Integration
    if (lowerPrompt.includes('what is mental illness') || lowerPrompt.includes('meaning of mental illness')) {
        await saveMessage('Auri', "Mental illnesses are health conditions that disrupt a person's thoughts, emotions, relationships, and daily functioning. It's important to know that they are medical conditions, not character flaws, and they are very much treatable. You're not alone in this. 💙");
        return;
    }

    if (lowerPrompt.includes('warning signs') || lowerPrompt.includes('symptoms of mental illness')) {
        await saveMessage('Auri', "Common warning signs include long-lasting sadness, extreme mood swings, excessive worrying, social withdrawal, and significant changes in eating or sleeping habits. If you're noticing these, it might be a good time to talk to a professional. 🩺");
        return;
    }

    if (lowerPrompt.includes('can i recover') || lowerPrompt.includes('recovery possible')) {
        await saveMessage('Auri', "Absolutely. With the right support—whether it's therapy, medication, or a strong support system—most people with mental health conditions can live full, productive, and happy lives. Recovery is a journey, not a destination! ✨");
        return;
    }

    if (lowerPrompt.includes('therapist vs psychiatrist') || lowerPrompt.includes('difference between therapist and psychiatrist')) {
        await saveMessage('Auri', "Great question! Generally, **psychiatrists** are medical doctors who can prescribe medication, while **therapists** (like psychologists or counselors) focus on talk therapy to help you process emotions and develop coping strategies. Often, the best approach involves both! 🤝");
        return;
    }

    if (lowerPrompt.includes('how to get help') || lowerPrompt.includes('where to start')) {
        await saveMessage('Auri', "Starting is the hardest part, but you've already taken the first step by asking! You can start by talking to someone you trust, or use our 'Aid' section to find professional resources. I'm also here to help you navigate your feelings every day. 🌈");
        return;
    }
...
function renderAuriView() {
    ...
    if (auriHistory.length > 0) {
        chatBubbles = auriHistory.map((msg, index) => {
            ...
            const sanitizedText = escapeHtml(msg.text || '');
    ...
    // Add focus/blur listeners to input for seamless nav transition
    const auriInput = document.getElementById('auri-input');
    const nav = document.getElementById('main-nav');
    if (auriInput && nav) {
        auriInput.addEventListener('focus', () => {
            // Keep nav visible or handle it more gracefully
            // nav.classList.add('nav-hidden'); // Removed this to keep nav visible
        });
        auriInput.addEventListener('blur', () => {
            // Only show if not scrolled down far
            if (APP_CONTENT.scrollTop < lastScrollTop || APP_CONTENT.scrollTop < NAV_THRESHOLD) {
                nav.classList.remove('nav-hidden');
            }
        });
    }
...
async function saveJournalEntry() {
    ...
    if (!window.db || !window.userId) {
        ...
        sessionStorage.setItem(CONFIG.STORAGE_KEYS.JOURNAL_ENTRIES, JSON.stringify(journalEntries));
...
async function deleteJournalEntry(entryId) {
    ...
    const { doc, deleteDoc, APP_ID } = window.firebaseImports || {};
    
    if (window.db && window.userId && doc && deleteDoc) {
        const docPath = `artifacts/${APP_ID}/users/${window.userId}/journal_entries/${entryId}`;
        try {
        ...
        // Local delete
        journalEntries = journalEntries.filter(e => e.id !== entryId && e.id !== entryId.toString());
        sessionStorage.setItem(CONFIG.STORAGE_KEYS.JOURNAL_ENTRIES, JSON.stringify(journalEntries));
        showNotification('Entry deleted locally', 'success');
        renderJournalView();
    }
}
...
function exportJournal() {
    if (typeof ExportService !== 'undefined') {
        ExportService.exportJournalAsJSON(journalEntries);
    } else {
        showNotification('Export service unavailable', 'error');
    }
}
        ...
        <div class="pt-6 animate-pageFadeIn">
            ...
            <div class="journal-notepad rounded-2xl shadow-2xl mb-8 card-hover">
                ...
                <textarea 
                    ...
                    onkeydown="if(event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); saveJournalEntry(); }"
...
function startBreathingExercise() {
    ...
    const totalCycles = CONFIG.BREATHING.CYCLES;
    ...
    function updateBreathing() {
        ...
        if (breathingState === 'in') {
            ...
            breathingState = 'hold';
            // Schedule next step based on CONFIG
            setTimeout(() => { if (breathingExerciseInterval) updateBreathing(); }, CONFIG.BREATHING.INHALE * 1000);
        } else if (breathingState === 'hold') {
            // Hold
            instructions.textContent = `⏸️ Hold...`;
            breathingState = 'out';
            setTimeout(() => { if (breathingExerciseInterval) updateBreathing(); }, CONFIG.BREATHING.HOLD * 1000);
            ...
            breathingState = 'rest';
            setTimeout(() => { if (breathingExerciseInterval) updateBreathing(); }, CONFIG.BREATHING.EXHALE * 1000);
        } else {
            // Rest
            circle.classList.remove('breathe-out');
            breathingCycle++;
            if (breathingCycle < totalCycles) {
                ...
                breathingState = 'in';
                setTimeout(() => { if (breathingExerciseInterval) updateBreathing(); }, 2000); // Small rest
            } else {
                updateBreathing(); // Trigger completion
            }
        }
    }
    
    instructions.textContent = "Get ready...";
    breathingExerciseInterval = true; // Mark as running
...
function generateAffirmation() {
    ...
    // Create a special pop-up or just use notification
    showNotification(`✨ ${randomAffirmation}`, 'success');
}

function openGratitudeJar() {
    changeView('journal');
    setTimeout(() => {
        const textarea = document.querySelector('.journal-notepad textarea');
        if (textarea) {
            textarea.placeholder = "Write 3 things you are grateful for today...";
            textarea.focus();
        }
    }, 400);
}
    ...
    APP_CONTENT.innerHTML = `
        <div class="pt-6 animate-pageFadeIn">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-gray-800 p-6 rounded-2xl shadow-xl border border-purple-600/50 card-hover col-span-1 md:col-span-2">
                    ...
                    <p class="text-sm text-gray-400 mb-4">A powerful technique to calm your nervous system instantly.</p>
                ...
                <div class="bg-gray-800 p-6 rounded-2xl shadow-xl card-hover">
                    ...
                    <p class="text-sm text-gray-400 mb-4">Reconnect with the present when anxiety hits hard.</p>
                    <ul class="text-xs text-gray-300 space-y-1 mb-4">
                        <li>👀 <strong>5</strong> things you see</li>
                        <li>✋ <strong>4</strong> things you feel</li>
                        <li>👂 <strong>3</strong> things you hear</li>
                    </ul>
                    <button onclick="startGroundingExercise()" class="w-full px-4 py-2 bg-sky-600 rounded-xl font-bold hover:bg-sky-700 transition-all duration-300 btn-hover text-sm">
                ...
                <div class="bg-gray-800 p-6 rounded-2xl shadow-xl card-hover">
                    ...
                    <p class="text-sm text-gray-400 mb-4">Question your negative thoughts using CBT principles.</p>
                    <button onclick="startThoughtChallenge()" class="w-full px-4 py-2 bg-orange-600 rounded-xl font-bold hover:bg-orange-700 transition-all duration-300 btn-hover text-sm">
                        Start Template
                    </button>
                </div>

                <!-- Positive Affirmations Card -->
                <div class="bg-gray-800 p-6 rounded-2xl shadow-xl card-hover border border-emerald-500/30">
                    <h3 class="text-xl font-semibold text-emerald-300 mb-2 flex items-center space-x-2">
                        <i data-lucide="heart" class="w-6 h-6"></i>
                        <span>Daily Affirmation</span>
                    </h3>
                    <p class="text-sm text-gray-400 mb-4">Get a quick boost of positivity and self-love.</p>
                    <button onclick="generateAffirmation()" class="w-full px-4 py-2 bg-emerald-600 rounded-xl font-bold hover:bg-emerald-700 transition-all duration-300 btn-hover text-sm">
                        Receive Affirmation
                    </button>
                </div>

                <!-- Gratitude Jar Card -->
                <div class="bg-gray-800 p-6 rounded-2xl shadow-xl card-hover border border-pink-500/30">
                    <h3 class="text-xl font-semibold text-pink-300 mb-2 flex items-center space-x-2">
                        <i data-lucide="star" class="w-6 h-6"></i>
                        <span>Gratitude Jar</span>
                    </h3>
                    <p class="text-sm text-gray-400 mb-4">Practicing gratitude is proven to improve long-term mood.</p>
                    <button onclick="openGratitudeJar()" class="w-full px-4 py-2 bg-pink-600 rounded-xl font-bold hover:bg-pink-700 transition-all duration-300 btn-hover text-sm">
                        Add to Jar
                    </button>
                </div>
            </div>
    ...
    APP_CONTENT.innerHTML = `
        <div class="pt-6 animate-pageFadeIn">
            ...
            <div class="bg-gray-800 p-6 rounded-2xl shadow-xl mb-6 card-hover">
                ...
                <div class="space-y-4">
                    ${CONFIG.INDIAN_HOTLINES.map(hotline => {
                        const isEmergency = hotline.type === 'emergency';
                        const numbers = hotline.numbers || [hotline.number];
                        return `
                            <div class="${isEmergency ? 'bg-red-900/30 border border-red-500/30' : 'bg-gray-900'} p-4 rounded-xl hover:bg-opacity-80 transition-all duration-300">
                                <div class="flex justify-between items-start mb-2">
                                    <div>
                                        <p class="font-bold ${isEmergency ? 'text-red-300' : 'text-white'}">${hotline.name}</p>
                                        <p class="text-sm ${isEmergency ? 'text-red-200/80' : 'text-gray-400'}">${hotline.description}</p>
                                    </div>
                                    <span class="text-xs ${isEmergency ? 'bg-red-800 text-red-100' : (hotline.hours === '24/7' ? 'bg-green-900 text-green-300' : 'bg-purple-900 text-purple-300')} px-2 py-1 rounded-full">${hotline.hours}</span>
                                </div>
                                <div class="${numbers.length > 1 ? 'grid grid-cols-2 gap-3' : ''}">
                                    ${numbers.map(num => `
                                        <a href="tel:${num.replace(/\s/g, '').replace(/-/g, '')}" class="${isEmergency ? 'text-xl bg-red-700 text-white' : 'text-lg bg-blue-900/30 text-blue-400'} font-bold hover:scale-[1.02] transition-all duration-300 block text-center py-2 rounded-lg">
                                            ${num}
                                        </a>
                                    `).join('')}
                                </div>
                                <p class="text-[10px] ${isEmergency ? 'text-red-300/70' : 'text-gray-500'} mt-2 text-center">
                                    ${hotline.type.toUpperCase()} | ${hotline.languages || (hotline.features ? hotline.features.join(' | ') : 'Support')}
                                </p>
                            </div>
                        `;
                    }).join('')}
...
function renderVentView() {
    const chatBubbles = ventMessages.map((msg, index) => {
        const isSelf = msg.userId === (VentSocket.myId || window.userId);
        const displayName = isSelf ? 'You' : `Anon-${(msg.userId || 'xxxx').substring(0, 4)}`;
        
        let dateObj;
        if (msg.timestamp instanceof Date) {
            dateObj = msg.timestamp;
        } else if (msg.timestamp && typeof msg.timestamp === 'object' && msg.timestamp.seconds) {
            ...
            <div class="vent-message" style="animation-delay: ${index * 50}ms">
                <div class="flex ${isSelf ? 'justify-end' : 'justify-start'} mb-4">
                    <div class="max-w-xs sm:max-w-md p-4 text-sm ${isSelf ? 'message-bubble-user' : 'message-bubble-auri'}">
                        ...
                        <p class="text-gray-100 whitespace-pre-wrap">${sanitizedText}</p>
    ...
    APP_CONTENT.innerHTML = `
        <div class="pt-4 flex flex-col h-full animate-pageFadeIn">
            <div class="flex justify-between items-center mb-4">
                ...
                <div id="vent-member-count" class="px-3 py-1 bg-purple-900/30 text-purple-400 rounded-full text-xs font-medium border border-purple-500/30">
                    ${VentSocket.memberCount || 1}/10 online
                </div>
            </div>
...
function handleProfileSave(event) {
    ...
    if (email && email !== oldEmail) {
        MeloEmailHandler.sendWelcomeEmail(email, name);
    }
}
...
function renderSettingsView() {
    const userId = window.uniqueUserId || window.userId || 'Not available';
...
function handleClearVentRoom() {
    if (confirm('Are you sure you want to clear all vent messages? This action cannot be undone.')) {
        ...
        ventMessages = [];
        localStorage.removeItem(CONFIG.STORAGE_KEYS.VENT_MESSAGES);
        sessionStorage.removeItem(CONFIG.STORAGE_KEYS.VENT_MESSAGES);
...
function initializeApp() {
    ...
    console.log("Firebase status:", {
        ...
        hasFirebaseImports: !!window.firebaseImports
    });
    
    // Check if we are on the app page
    if (!APP_CONTENT) {
        console.error("❌ APP_CONTENT element not found. Skipping app initialization.");
        return;
    }
    ...
    try {
        const savedAuriHistory = sessionStorage.getItem(CONFIG.STORAGE_KEYS.AURI_HISTORY) || localStorage.getItem(CONFIG.STORAGE_KEYS.AURI_HISTORY);
        const savedJournalEntries = sessionStorage.getItem(CONFIG.STORAGE_KEYS.JOURNAL_ENTRIES) || localStorage.getItem(CONFIG.STORAGE_KEYS.JOURNAL_ENTRIES);
        const savedVentMessages = sessionStorage.getItem(CONFIG.STORAGE_KEYS.VENT_MESSAGES) || localStorage.getItem(CONFIG.STORAGE_KEYS.VENT_MESSAGES);
        const savedUserProfile = sessionStorage.getItem(CONFIG.STORAGE_KEYS.USER_PROFILE) || localStorage.getItem(CONFIG.STORAGE_KEYS.USER_PROFILE);
        ...
        if (auriHistory.length === 0) {
            auriHistory.push({ ...WELCOME_MESSAGE });
        }
    } catch (error) {
    ...
    const displayId = window.uniqueUserId || window.userId;
    if (displayId) {
        const userIdDisplay = document.getElementById('user-id-display');
        if (userIdDisplay) {
            userIdDisplay.textContent = `ID: ${displayId.substring(0, 8)}...`;
            userIdDisplay.title = `Full ID: ${displayId}`;
    ...
    console.log("✅ Melo fully initialized!");
}
...
setTimeout(() => {
    if (!appInitialized) {
        console.warn('⚠️ Auth event did not fire within 3s. Initializing app in local-only mode.');
        if (window.firebaseConfigError) {
            console.error('❌ Firebase config module failed to load.');
        }
        initializeApp();
    }
}, 3000);
...
window.handleAuriSubmit = handleAuriSubmit;
window.setMood = setMood;
window.saveJournalEntry = saveJournalEntry;
window.deleteJournalEntry = deleteJournalEntry;
window.exportJournal = exportJournal;
window.changeViewWithAnimation = changeViewWithAnimation;
window.handleVentSubmit = handleVentSubmit;
...
window.handleClearVentRoom = handleClearVentRoom;
window.startBreathingExercise = startBreathingExercise;
window.startGroundingExercise = startGroundingExercise;
window.startThoughtChallenge = startThoughtChallenge;
window.exportJournalEntry = exportJournalEntry;
window.showChangePasswordDialog = showChangePasswordDialog;
window.getMoodAnalysis = getMoodAnalysis;
